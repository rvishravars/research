...
// These definitions MUST match the driver (asgn1_skel.c)
...
int main (int argc, char **argv)
{
    ...
    *(mmap_buf + j) = random () % j;
    ...
    if (ioctl (fd, ASGN1_IOCTL_SET_MAX_USERS, &nproc) < 0) {
        fprintf (stderr, "ioctl failed:  %s\n", strerror (errno));
        exit (1);
    }
    ...
    int current_max = 0;
    if (ioctl (fd, ASGN1_IOCTL_GET_MAX_USERS, &current_max) < 0) {
        fprintf (stderr, "ioctl GET_MAX_USERS failed: %s\n", strerror(errno));
        exit(1);
    }
    printf("Driver now reports max_users = %d\n", current_max);
    assert(current_max == nproc);
