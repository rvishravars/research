...
typedef struct {
    int gpio;
    int flag; // direction and value flags
    char label[10];    
} legacy_gpio;

#define GPIO_ARRAY_SIZE 10

static legacy_gpio gpio_dummy[GPIO_ARRAY_SIZE] = {
                { 519, GPIOF_IN, "GPIO7" },
...
static int gpio_request_array(legacy_gpio *ga, int size)
{
int i, ret;

    for(i=0; i<size; i++){
	ret = gpio_request_one(ga[i].gpio, ga[i].flag, ga[i].label);
	if(ret != 0) break;
    }
    return ret;
}
...
static void gpio_free_array(legacy_gpio *ga, int size)
{
int i;
...
int gpio_dummy_init(void)
{
    ...
    ret = gpio_request_array(gpio_dummy, GPIO_ARRAY_SIZE);

    if (ret) {
	printk(KERN_ERR "Unable to request GPIOs for the dummy device: %d\n", ret);
	return ret;
        }
    ret = gpio_to_irq(gpio_dummy[GPIO_ARRAY_SIZE-1].gpio);
    if(ret < 0) {
	printk(KERN_ERR "Unable to request IRQ for gpio %d: %d\n", gpio_dummy[GPIO_ARRAY_SIZE-1].gpio, ret);
        goto fail1;
    }
    dummy_irq = ret;
    printk(KERN_WARNING "Successfully requested IRQ# %d for %s\n", dummy_irq, gpio_dummy[GPIO_ARRAY_SIZE-1].label);
...
fail1:
    gpio_free_array(gpio_dummy, GPIO_ARRAY_SIZE);
